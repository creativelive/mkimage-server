...
function im_info(file, force, callback) {
  ...
  var imcmd = 'identify -precision 16 -format "%m\n%z\n%w\n%h\n%b\n%f" ' + file,
...
function im_resize(options, force, callback) {
  ...
  const imcmd = sprintf('convert -interlace Plane %s -resize "%sx%s"%s -quality %d %s', options.src, options.width, options.height, options.adjustment, options.quality, options.dst);
  exec(imcmd, function(err, stdout, stderr) {
