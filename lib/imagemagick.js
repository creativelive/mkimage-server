...
function im_resize(options, force, callback) {
  ...
  let preformatted = 'magick %s -interlace Plane -resize "%sx%s"%s -quality %d +profile "!icc,*" ';
...
function im_crop(options, force, callback) {
  ...
  // default is 92 to avoid downsampling chroma channels
  options.quality = options.quality || 92;
  var preformatted = 'magick %s -interlace Plane -gravity %s -crop %sx%s+%d+%d -quality %d +profile "!icc,*" ';
  if (options.format === 'webp') {
    preformatted += '-define webp:lossless=false ';
  }
  preformatted += '%s';
...
function im_chop(options, force, callback) {
  ...
  var preformatted = 'magick %s -interlace Plane -resize "%sx%s^" -gravity center -crop %sx%s+0+0 -quality %d +profile "!icc,*" ';
  if (options.format === 'webp') {
    preformatted += '-define webp:lossless=false ';
  }
  preformatted += '%s';
