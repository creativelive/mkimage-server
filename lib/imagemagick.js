...
var im_info_lru = new lru(10240); // 10240 max items
...
function im_resize(options, force, callback) {
  ...
  var imcmd = sprintf('convert -interlace Plane %s -resize "%sx%s"%s -quality %d %s', options.src, options.width, options.height, options.adjustment, options.quality, options.dst);
  exec(imcmd, function(err, stdout, stderr) {
...
function im_add_play_button(options, force, callback) {
  ...
  if (!options.dst) {
    return callback(new Error('dst must not be ommited'));
  }
...
function im_crop(options, force, callback) {
  ...
  var imcmd = sprintf('convert -interlace Plane %s -gravity %s -crop %sx%s+%d+%d -quality %d %s', options.src, options.gravity, options.width, options.height, options.x, options.y, options.quality, options.dst);
  exec(imcmd, function(err, stdout, stderr) {
    if (err) return callback(err);
    im_info(options.dst, force, callback);
  });
...
function im_chop(options, force, callback) {
  ...
  var imcmd = sprintf('convert -interlace Plane %s -resize "%sx%s^" -gravity center -crop %sx%s+0+0 -quality %d %s', options.src, options.width, options.height, options.width, options.height, options.quality, options.dst);
  console.log(imcmd);
  exec(imcmd, function(err, stdout, stderr) {
    if (err) return callback(err);
    im_info(options.dst, force, callback);
