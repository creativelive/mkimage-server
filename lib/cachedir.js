...
class Cachedir {
   ...
   */
  get_working_path(filename, callback) {
    const workingFile = tag(path.basename(filename));
    const dest = sprintf('%s/%s', this.work_dir, workingFile);
   ...
   */
  promote_working_path_to_cache(workingPath, callback) {
    const workingFile = untag(path.basename(workingPath));
