...
const {AppError, BadRequestError, ForbiddenError, ServerError} = require('./errors');
...
//      log.warn('request ' + req.url + ' is not valid');
      ...
      return next(new BadRequestError('missing command'));
    }
  ...
  function processImage(params, method, log, callback) {
    ...
    imgcache.download(params.url, force, log, (err, downloaded_image, downloadExists) => {
      ...
      imgcache.get_cached_path(filename, (dest, exists) => {
        ...
        imgcache.get_working_path(filename, (workingPath, workingExists) => {
          imgcache.ensure_path(workingPath, (err) => {
            ...
            imagemagick[method]({
              ...
              imgcache.promote_working_path_to_cache(workingPath, (err, cachePath) => {
                if (err) {
                  callback(err);
                  return;
                }
